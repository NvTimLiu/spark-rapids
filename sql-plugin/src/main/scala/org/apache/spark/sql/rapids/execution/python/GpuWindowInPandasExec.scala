...
/*
 * This GpuWindowInPandasExecBase aims at supporting running Pandas UDF code
 * on GPU at Python side.
 *
 * (Currently it will not run on GPU itself, since the columnar way is not implemented yet.)
 *
 */
abstract case class GpuWindowInPandasExecBase(
  ...
  def getWindowFrameExpressionFactoryPairs():(Seq[Expression], Seq[UnboundedWindowFunctionFrame]) = {
    val exp = Nil
    val fac = Nil
    (exp, fac)
  }
...
/*
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    val expressionsWithFrameIndex =
      getWindowFrameExpressionFactoryPairs().map(_._1).zipWithIndex.flatMap {
    ...
    val factories = getWindowFrameExpressionFactoryPairs().map(_._2).toArray
