...
DBFS_FILEPATH=/dbfs/FileStore/tim/
sudo rm -rf $DBFS_FILEPATH && sudo mkdir -p $DBFS_FILEPATH
echo "Copying rapids jars: dist/target/$RAPIDS_BUILT_JAR $DBFS_FILEPATH"
sudo cp dist/target/$RAPIDS_BUILT_JAR $DBFS_FILEPATH
echo "Copying cudf jars: $CUDF_JAR $DBFS_FILEPATH"
sudo cp $CUDF_JAR $DBFS_FILEPATH
