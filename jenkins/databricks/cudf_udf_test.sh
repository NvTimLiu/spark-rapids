...
#  cudf conda packages are needed to run the script, please refer to
# '`./jenkins/databricks/init_cudf_udf.sh' for details.
...
if [ !-d "${CONDA_HOME}/envs/cudf-udf" ]; then
    echo "Error not found cudf conda packages, please install via 'init_cudf_udf.sh'"
    exit -1
fi
...
PYTHON_SITE_PACKAGES="${CONDA_HOME}/envs/cudf-udf/lib/${sw_versions[PYTHON]}/site-packages"
