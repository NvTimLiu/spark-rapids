...
for VER in ${VERSIONS_LIST}; do
    ...
    $DEPLOY_CMD -Durl=$SERVER_URL -DrepositoryId=$SERVER_ID \
            $TESTS_DOC_JARS \
            -Dfile=$TESTS_FPATH.jar -Dclassifier=spark$VER -DpomFile=${TESTS_PL}/pom.xml
done

###### Deploy profiling tool jar(s) ######
TOOL_PL=${TOOL_PL:-"tools"}
TOOL_ART_ID=`mvn help:evaluate -q -pl $TOOL_PL -Prelease311 -Dexpression=project.artifactId -DforceStdout`
TOOL_ART_VER=`mvn help:evaluate -q -pl $TOOL_PL -Prelease311 -Dexpression=project.version -DforceStdout`
...
cp $TOOL_FPATH.jar ${TOOL_FPATH}-sources.jar ; cp  $TOOL_FPATH.jar ${TOOL_FPATH}-javadoc.jar ######### 
$DEPLOY_CMD -Durl=$SERVER_URL -DrepositoryId=$SERVER_ID \
            $TOOL_DOC_JARS \
